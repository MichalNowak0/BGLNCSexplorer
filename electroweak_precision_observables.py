...
#-------------------------------------------------------------------------------------------------
# Auxilliary functions used to compute the oblique parameters
#-------------------------------------------------------------------------------------------------
...
def G(I, J, Q):
    ...
    return - 16/3 + 5*(I + J)/Q - 2*(I - J)**2/(Q**2) + fourthTerm + r/(Q**3)*smallF(t, r)
...
def gTilde(I, J, Q):
    ...
    t = I + J - Q
    r = Q**2 - 2*Q*(I + J) + (I - J)**2
    return - 2 + ((I - J)/Q - (I + J)/(I - J))*np.log(I/J) + smallF(t, r)/Q

def gHat(I, Q):
    return G(I, Q, Q) + 12*gTilde(I, Q, Q)

#---------------------------------------------------------------------------------------------------
# Function for demonstration purposes
#---------------------------------------------------------------------------------------------------

def demonstrate_identities(U, V, v, v1, v2):
...
def t_parameter(g, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh):
    
    firstTermT = abs(uDv[1][0])**2*f(m_chh**2, m_hh1**2) + abs(uDv[1][1])**2*f(m_chh**2, m_hh2**2) + \
        abs(uDv[1][3])**2*f(m_chh**2, m_ah**2) + abs(uDv[1][4])**2*f(m_chh**2, m_hh3**2) + \
        abs(uDv[1][5])**2*f(m_chh**2, m_pgs**2)
    secondTermT = np.imag(vDv[0][1])**2*f(m_hh1**2, m_hh2**2) + np.imag(vDv[0][3])**2*f(m_hh1**2, m_ah**2) + \
        np.imag(vDv[0][4])**2*f(m_hh1**2, m_hh3**2) + np.imag(vDv[0][5])**2*f(m_hh1**2, m_pgs**2) + \
        np.imag(vDv[1][3])**2*f(m_hh2**2, m_ah**2) + np.imag(vDv[1][4])**2*f(m_hh2**2, m_hh3**2) + \
        np.imag(vDv[1][5])**2*f(m_hh2**2, m_pgs**2) + np.imag(vDv[3][4])**2*f(m_ah**2, m_hh3**2) + \
        np.imag(vDv[3][5])**2*f(m_ah**2, m_pgs**2) + np.imag(vDv[4][5])**2*f(m_hh3**2, m_pgs**2)
    # The third term is zero since we only have one physical charged scalar
    thirdTermT = 0 
    fourthTermT = np.imag(vDv[2][0])**2*(f(m_Z**2, m_hh1**2) - f(m_W**2, m_hh1**2)) + \
        np.imag(vDv[2][1])**2*(f(m_Z**2, m_hh2**2) - f(m_W**2, m_hh2**2)) + \
        np.imag(vDv[2][3])**2*(f(m_Z**2, m_ah**2) - \
        f(m_W**2, m_ah**2)) + np.imag(vDv[2][4])**2*(f(m_Z**2, m_hh3**2) - f(m_W**2, m_hh3**2)) + \
        np.imag(vDv[2][5])**2*(f(m_Z**2, m_pgs**2) - f(m_W**2, m_pgs**2))
    fifthTermT = f(m_Z**2, m_H**2) - f(m_W**2, m_H**2) 
...
def s_parameter(g, wAngle, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh):
    firstTermS = (2*np.sin(wAngle)**2 - uDu[1][1])**2*G(m_chh**2, m_chh**2, m_Z**2)
    secondTermS = 0
    thirdTermS = np.imag(vDv[0][1])**2*G(m_hh1**2, m_hh2**2, m_Z**2) + np.imag(vDv[0][3])**2*G(m_hh1**2,\
                         m_ah**2, m_Z**2) + np.imag(vDv[0][4])**2*G(m_hh1**2, m_hh3**2, m_Z**2) + \
                         np.imag(vDv[0][5])**2*G(m_hh1**2, m_pgs**2, m_Z**2) + np.imag(vDv[1][3])**2*G(m_hh2**2,\
                         m_ah**2, m_Z**2) + np.imag(vDv[1][4])**2*G(m_hh2**2, m_hh3**2, m_Z**2) + \
                         np.imag(vDv[1][5])**2*G(m_hh2**2, m_pgs**2, m_Z**2) + np.imag(vDv[3][4])**2*G(m_ah**2,\
                         m_hh3**2, m_Z**2) + np.imag(vDv[3][5])**2*G(m_ah**2, m_pgs**2, m_Z**2) + \
                         np.imag(vDv[4][5])**2*G(m_hh3**2, m_pgs**2, m_Z**2)
    ...
    sixthTermS = np.imag(vDv[2][0])**2*(gHat(m_hh1**2, m_Z**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][1])**2*(gHat(m_hh2**2, m_Z**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][3])**2*(gHat(m_ah**2, m_Z**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][4])**2*(gHat(m_hh3**2, m_Z**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][5])**2*(gHat(m_pgs**2, m_Z**2) - gHat(m_H**2, m_Z**2))
...
def u_parameter(g, wAngle, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh):
    firstTermU = abs(uDv[1][0])**2*G(m_chh**2, m_hh1**2, m_W**2) + \
                 abs(uDv[1][1])**2*G(m_chh**2, m_hh2**2, m_W**2) + \
                 abs(uDv[1][3])**2*G(m_chh**2, m_ah**2, m_W**2) + \
                 abs(uDv[1][4])**2*G(m_chh**2, m_hh3**2, m_W**2) + \
                 abs(uDv[1][5])**2*G(m_chh**2, m_pgs**2, m_W**2)
    secondTermU = (2*np.sin(wAngle)**2 - uDu[1][1])**2*G(m_chh**2, m_chh**2, m_Z**2)
    thirdTermU = 0
    fourthTermU = np.imag(vDv[0][1])**2*G(m_hh1**2, m_hh2**2, m_Z**2) + np.imag(vDv[0][3])**2*G(m_hh1**2,\
                         m_ah**2, m_Z**2) + np.imag(vDv[0][4])**2*G(m_hh1**2, m_hh3**2, m_Z**2) + \
                         np.imag(vDv[0][5])**2*G(m_hh1**2, m_pgs**2, m_Z**2) + np.imag(vDv[1][3])**2*G(m_hh2**2,\
                         m_ah**2, m_Z**2) + np.imag(vDv[1][4])**2*G(m_hh2**2, m_hh3**2, m_Z**2) + \
                         np.imag(vDv[1][5])**2*G(m_hh2**2, m_pgs**2, m_Z**2) + np.imag(vDv[3][4])**2*G(m_ah**2,\
                         m_hh3**2, m_Z**2) + np.imag(vDv[3][5])**2*G(m_ah**2, m_pgs**2, m_Z**2) + \
                         np.imag(vDv[4][5])**2*G(m_hh3**2, m_pgs**2, m_Z**2)
    fifthTermU = np.imag(vDv[2][0])**2*(gHat(m_hh1**2, m_W**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][1])**2*(gHat(m_hh2**2, m_W**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][3])**2*(gHat(m_ah**2, m_W**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][4])**2*(gHat(m_hh3**2, m_W**2) - gHat(m_H**2, m_Z**2)) + \
                 np.imag(vDv[2][5])**2*(gHat(m_pgs**2, m_W**2) - gHat(m_H**2, m_Z**2))
    sixthTermU = gHat(m_H**2, m_Z**2) - gHat(m_H**2, m_W**2)
...
def electroweakPresObs(beta, a1, a2, a3, gamma1, v, v1, v2, v3, l1, l2, l3, l4, l1d, l2d, l3d, mu3, mub,
    ...
    T = t_parameter(g, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh)
    #print("T is: ", T)
    ...
    S = s_parameter(g, wAngle, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh)
    #print("S is: ", S)
    ...
    U = u_parameter(g, wAngle, m_Z, m_W, m_H, uDv, uDu, vDv, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh)
    #print("U is: ", U)
