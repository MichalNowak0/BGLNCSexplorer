...
import inversion_procedure as ip
import file_writing as fw
import electroweak_precision_observables as epo

def makeNewPointFolder(counter, thisRunDir):
...
def simulationLoop(thisRunDir, v_3_sRange, beta_sRange, a1_sRange, gamma1_sRange, delta_2_sRange, 
                   delta_3_sRange, m_hh2_sRange, m_hh3_sRange, m_ah_sRange, m_pgs_sRange, m_chh_sRange,
                   t_range, s_range, u_range, sarah_model_version, spheno_version, higgs_bounds_version,
                   higgs_signals_version, maxRunNum, spheno_BGL, HiggsBounds, HiggsSignals, Running_Env,
                   Result_data):
    
    for counter in range(maxRunNum):
        # Make a new folder for this parameter-point. Named with the counter integer:
        thisPointDir = makeNewPointFolder(counter, thisRunDir)
        ...
        #-------------------------------------------------------------------------------------------------------------------
        fast_Aa1, fast_Mub, fast_Mu3, fast_Lambda1, fast_Lambda2, fast_Lambda4, fast_Lambda3, \
        fast_Lambda1Dash, fast_Lambda2Dash, fast_Lambda3Dash, fast_Mu1, fast_Mu2, fast_MuDash = ip.initialize_symb_expr()
        num_Aa1, num_Mub, num_Mu3, num_Lambda1, num_Lambda2, num_Lambda3, num_Lambda4, num_Lambda1Dash, num_Lambda2Dash, \
        num_Lambda3Dash, num_Mu1, num_Mu2, num_MuDash = ip.scalar_inversion(num_gamma1, num_Aa1, num_Aa2, num_Aa3,
                                                                              num_Aa4, m_hh1, m_hh2, m_hh3, m_ah, m_pgs, 
                                                                              m_chh, num_v, num_v_1, num_v_2, num_v_3,
                                                                              num_a1, num_a2, num_a3, fast_Aa1, fast_Mub, fast_Mu3, fast_Lambda1, fast_Lambda2, fast_Lambda4, fast_Lambda3,
        ...
        t, s, u, matrixScal, matrixPs, matrixChS = epo.electroweakPresObs(num_Beta, num_a1, num_a2, num_a3, num_gamma1,
        ...
        """
        flag = 0       # Records how far the point got through the checks & programs
                ...
                if four_bar > 0:
                    ...
                    if abs(2*num_Lambda1) < Upl and abs(2*num_Lambda2) < Upl and abs(num_Lambda3) < Upl and \
                        ...
                        if t > t_range[0] and t < t_range[1] and s > s_range[0] and s < s_range[1] and u > u_range[0] \
                            ...
                            Y1u22, Y2u33, vlu, vru, vld, vrd = ip.Quark_Yukawa_Inversion(num_v, num_Beta, num_v_1,\
                                                                                           num_v_2, num_v_3)
                            ...
                            B22, C13, C23, C31, C32, vll, vrl = ip.Lepton_Yukawa_Inversion(num_v, num_v_1, \
                                                                                             num_v_2, num_v_3)
    
                            # Create the SPheno input file:
                            fw.write_spheno_LesHouches(sarah_model_version, Running_Env, 
                            ...
                            Spheno_BGL_Running_command = spheno_BGL + ' ' + os.path.join(Running_Env,'spheno',
                                                                'LesHouches.in.{}'.format(sarah_model_version))
                            os.system(Spheno_BGL_Running_command)
                            
                            # Run the rest of the loop only if SPheno successfully produced output:
                            data_spheno = os.path.join(os.getcwd(), 'SPheno.spc.{}'.format(sarah_model_version))
                            if os.path.exists(data_spheno):
                                
                                flag = 5
    
                                # Save masses to compare to SPheno output:
                                fw.saveParamsForComparison(sarah_model_version, Running_Env, 
                                ...
                                HiggsBounds_running_command = HiggsBounds + ' LandH effC 5 1 ' + thisPointDir + '/'
                                os.system(HiggsBounds_running_command)
        
                                # create the HiggsSignals output
                                HiggsSignals_running_command = HiggsSignals + ' latestresults peak 2 effC 5 1 ' + \
                                                                thisPointDir + '/'
        ...
        # Regardless of how many checks the point passed, record the preSpheno data AND the "flag" variable:
        fw.savePreSPhenoParams(m_hh1, m_hh2, m_hh3, m_ah, m_pgs, m_chh, num_gamma1, num_Mub, num_Mu3, num_Lambda1,
                                 num_Lambda2, num_Lambda3, num_Lambda4, num_Lambda1Dash, num_Lambda2Dash, num_Lambda3Dash,
                                 num_Mu1, num_Mu2, num_MuDash, num_v, num_v_1, num_v_2, num_v_3, num_Beta, num_a1, num_a2,
                                 num_a3, delta_2, delta_3, s, t, u, num_Aa1, flag)
#---------------------------------------------------------------------------------------------------------------------------------                              
